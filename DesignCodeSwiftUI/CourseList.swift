...
struct CourseList: View {
    ...
    var body: some View {
                ...
                ForEach(courses.indices, id: \.self) { index in
                    GeometryReader { geomtry in
                        CourseView(
                            ...
                            index: index)
                            .offset(y: self.courses[index].show ? -geomtry.frame(in: .global).minY : 0)
                            .opacity(self.activeIndex != index && self.isActive ? 0 : 1)
                            .scaleEffect(self.activeIndex > index && self.isActive ? 0.5 : 1)
                            .offset(y: self.activeIndex < index && self.isActive ? screen.height : 0)
                    }
                    .frame(height: 280)
        ...
        .statusBar(hidden: isActive ? true : false)
        .animation(.linear)
    }
}
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            .gesture(DragGesture().onChanged({ (value) in
                guard self.activeView.height < 300 else { return }
                guard self.activeView.height > 0 else { return }
